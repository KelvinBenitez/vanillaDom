...
function clickCoffee(data) {
  ...
  if (unlockedProducers.length > 0) {
    unlockedProducers.forEach(function (producer) {
      renderProducerView(producer);
    });
  }
}
...
function unlockProducers(producers, coffeeCount) {
  producers.forEach((producer) => {
    if (producer.unlocked === false && coffeeCount >= producer.price / 2) {
...
function makeDisplayNameFromId(id) {
  ...
  const captWords = words.map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase());
  return captWords.join(' ');
}
...
function buyButtonClick(event, data) {
  if (event.target.tagName !== 'BUTTON' || !event.target.id.includes('buy_')) {
    return;
  }
